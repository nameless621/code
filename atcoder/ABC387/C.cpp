...
int main() {
    ...
    auto f = [&](ll X) {
        ...
        reverse(all(p));
        int n = p.size();
        // 桁数、先頭の数字、最後に選んだ数字、R未満かどうか
        vector dp(n, vector(10, vector(10, vector<ll>(2))));
        dp[0][p[0]][p[0]][0] = 1;
        reps(i,1,p[0]) dp[0][i][i][1] = 1;
        rep(i,n-1) {
            if(i < n - 2) reps(j,1,10) dp[i+1][j][j][1] += 1;
            reps(j,1,10) rep(k,10) rep(l,2) {
                int x = p[i+1];
                // 次に選ぶ数字
                rep(d,10) {
                    int nl = l;
                    ...
                    if(j <= d) continue;
                    dp[i+1][j][d][nl] += dp[i][j][k][l];
                }
            }
        }
        ll res = 0;
        rep(j,10) rep(k,10) rep(l,2) res += dp[n-1][j][k][l];
        return res;
    };
